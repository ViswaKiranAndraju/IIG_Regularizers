...
class BalancedFTRLShannon(OMDBase):
    """A class for Balanced FTRL algorithm with Shannon entropy regularization.
    ...
    def _shannon_entropy(self, p):
        return -np.sum(p * np.log(p + 1e-12))  # Shannon entropy calculation

    def update_policy_with_shannon_entropy(self):
        for state_key in range(self.policy_shape[0]):
            policy_logits = self.current_logit[state_key, :]
            probabilities = np.exp(policy_logits) / np.sum(np.exp(policy_logits))
            entropy = self._shannon_entropy(probabilities)
            policy_logits -= self.learning_rates[state_key] * entropy
            self.current_logit[state_key, :] = policy_logits
        self.current_policy.action_probability_array = np.exp(self.current_logit)
    ...
    def compute_balanced(self):
        ...
        self.legal_actions_from_key = [[] for _ in range(self.policy_shape[0])]
        self.balanced_policy = np.zeros(self.policy_shape)
        self.balanced_transition_plan = np.zeros(self.policy_shape[0], dtype=float)
        self.key_children = [defaultdict(list) for _ in range(self.policy_shape[0])]
    ...
    def compute_information_tree_from_state(self, state, trajectory, depth):
        ...
        if self.total_actions_from_key[state_key] == 0:
            ...
            if len(trajectory[current_player]) == 0:
                ...
                self.key_children[trajectory[current_player][-1][0]][trajectory[current_player][-1][1]].append(
                    state_key)
    ...
    def compute_balanced_policy_from_key(self, state_key):
        for action in self.legal_actions_from_key[state_key]:
            self.balanced_policy[state_key, action] = self.total_actions_from_action[state_key, action] / \
                                                      self.total_actions_from_key[state_key]
    ...
    def compute_balanced_transition_from_action(self, state_key, action, current_transition):
        if self.total_actions_from_action[state_key, action] > 1:
            for state_key_child in self.key_children[state_key][action]:
                new_transition = current_transition * self.total_actions_from_key[state_key_child] / \
                                 self.total_actions_from_action[state_key, action]
