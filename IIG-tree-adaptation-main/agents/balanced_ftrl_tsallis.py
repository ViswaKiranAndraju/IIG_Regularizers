...
class BalancedFTRLTsallis(OMDBase):
    """A class for Balanced FTRL algorithm using Tsallis Entropy regularizer."""

    def __init__(
            self,
            game,
            budget,
            q_param=1.5,  # Tsallis q parameter, 1.0 reduces to Shannon entropy
            base_constant=1.0,
            lr_constant=1.0,
            lr_pow_H=-0.5,
            lr_pow_A=-0.5,
    ...
    def compute_balanced_policy_from_key(self, state_key):
        for action in self.legal_actions_from_key[state_key]:
            prob = self.total_actions_from_action[state_key, action] / self.total_actions_from_key[state_key]
            self.balanced_policy[state_key, action] = self.tsallis_regularizer(prob)
            for state_key_child in self.key_children[state_key][action]:
                self.compute_balanced_policy_from_key(state_key_child)

    def tsallis_regularizer(self, prob):
        """Applies the Tsallis regularization to a probability value."""
        q = self.q_param
        if q == 1.0:
            return prob  # Reduces to Shannon entropy
        return (prob ** q) / (q - 1)
